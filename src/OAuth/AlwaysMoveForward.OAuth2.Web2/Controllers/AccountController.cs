...
using AlwaysMoveForward.OAuth2.Web2.Models;
using AlwaysMoveForward.OAuth2.Web2.Models.AccountViewModels;
...
namespace AlwaysMoveForward.OAuth2.Web2.Controllers
{
    ...
    public class AccountController : AMFControllerBase
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                AMFUserLogin registerUser = this.ServiceManager.UserService.GetByEmail(model.Email);
        ...
        public async Task<IActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                ...
                AMFUserLogin externalLoginUser = this.ServiceManager.UserService.GetByEmail(model.Email);

                if (externalLoginUser == null)
                {
                    externalLoginUser = new AMFUserLogin();
                    externalLoginUser.Email = model.Email;
                    externalLoginUser.FirstName = "FirstName";
                    externalLoginUser.LastName = "LastName";
                    externalLoginUser.Role = RoleType.Id.User;
                    externalLoginUser.UserStatus = UserStatus.Active;
                }

                var result = await _userManager.CreateAsync(externalLoginUser);

                if (result.Succeeded)
                {
                    result = await _userManager.AddLoginAsync(externalLoginUser, info);
                    if (result.Succeeded)
                    {
                        await _signInManager.SignInAsync(externalLoginUser, isPersistent: false);
