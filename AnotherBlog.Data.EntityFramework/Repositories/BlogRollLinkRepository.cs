...
namespace AnotherBlog.Data.EntityFramework.Repositories
{
    ...
    /// </summary>
    public class BlogRollLinkRepository : EntityFrameworkRepository<BlogRollLink, IBlogRollLink>, IBlogRollLinkRepository
    {
        ...
        public BlogRollLink GetByUrlAndBlogId(int blogId, string url)
        {
            ...
            try
            {
                retVal = (from foundItem in ((UnitOfWork)this.UnitOfWork).DataContext.BlogRollLinkDTOs where foundItem.Blog.BlogId == blogId && foundItem.Url == url select foundItem).Single();
            }
            catch (Exception e)
            {
